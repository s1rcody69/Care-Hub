...
const AddAppointment = () => {
  ...
  const requiredFilled = [form.patientId, form.date, form.time].filter(
    Boolean,
  ).length;
              ...
              {[
                ...
                <div key={label} className="flex items-center gap-2 py-1">
                  <CheckCircle2
                    size={14}
                    className={
                      filled ? "text-emerald-500" : "text-surface-border"
                    }
                  />
                  <span
                    className={`text-xs ${filled ? "text-emerald-700 font-medium" : "text-ink-muted"}`}
                  >
                    {label}
                  </span>
                </div>
